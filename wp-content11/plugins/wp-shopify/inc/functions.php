<?php
	...
	function register_wpshopify_scripts(){
		...
		if(array_key_exists('page', $_GET) && $_GET['page']=='wp_shopify'){
			...
			if(is_admin()){			
				wp_enqueue_style('wp-shopify-styles', plugins_url('css/admin-styles.css', dirname(__FILE__)), array(), date('Ymdhi'), 'all' );
				...
				wp_enqueue_style('fontawesome', plugins_url('css/fontawesome.min.css', dirname(__FILE__)), array(), date('Ymd'), 'all' );
				...
				wp_enqueue_style('bootstrap', plugins_url('css/bootstrap.min.css', dirname(__FILE__)), array(), date('Ymd'), 'all' );
				...
				wp_enqueue_style('wp-shopify-styles', plugins_url('css/front-styles.css', dirname(__FILE__)), array(), date('Ymd'), 'all' );
			}
	...
	function wpsy_shop_display_shortcode($atts = array(), $content = null, $tag = ''){
		...
		switch($type){
			...
			case 'collection':		
				if($id){					
					$query_params = array('query'=>'collection', 'id'=>$id);
					$store_data = wpsy_graphql_central($query_params, true);

					$store_collection = (!empty($store_data)?$store_data->collection:array());
...
<?php					
					...
					$store_data = (!empty($store_collection)?$store_collection->products->edges:array());
				}
	...
	?>
	<li>
	<a title="<?php echo ucwords($product_data->productType); ?>" href="<?php echo $product_url; ?>"><img alt="<?php echo $product_data->title; ?>" src="<?php echo $featuredImage; ?>" /></a>
	...
	function wpsy_shop_product_display_shortcode($atts = array(), $content = null, $tag = ''){
		...
		if($wpsy_pro && $template!='' && $template!='default'){
			...
			$o .= '<div class="prod-right">' . '<a target="_blank" href="' . $data['link'] . '"><h3>' . $data['name'] . '</h3></a>' . '<br />';
			$o .= '<div class="prod-price">' .$data['price']. '</div>';
			$o .= '<div class="prod-buy"><a href="' . $data['link'] . '" target="_blank"><img src="'.plugins_url( 'images/btn.png', dirname(__FILE__) ).'" /></a></div>';
			$o .= '</div>';//prod-right
			$o .= '</div>';//prod-upper
	...
	{
	   wp_register_style('shop_product_display', plugins_url('css/front-styles.css', dirname(__FILE__)));
